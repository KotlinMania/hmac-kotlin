# Claude Code Project Instructions — hmac-kotlin

## Project Overview

This is **hmac-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`hmac`](https://crates.io/crates/hmac).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/hmac/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/hmac>
