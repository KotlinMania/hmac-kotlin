...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Hmac.swiftmodule/`
...
//      `import Hmac` above would fail at compile time.
//
//   2. The static archive `libHmac.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Hmac" }` and
//      `flattenPackage = "io.github.kotlinmania.hmac"` configuration in
...
final class HmacExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Hmac swift module imported cleanly")
    }
}
