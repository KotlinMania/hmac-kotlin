...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`hmac`](https://crates.io/crates/hmac).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `hmac` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Generic HMAC-with-Digest implementation
...
```kotlin
dependencies {
    implementation("io.github.kotlinmania:hmac-kotlin:0.1.0-SNAPSHOT")
}
```
...
Original work copyrighted by the upstream `hmac` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
